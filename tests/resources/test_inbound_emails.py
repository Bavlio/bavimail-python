...
def test_list_inbound_emails_with_include_bounce() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        assert request.method == "GET"
        assert request.url.params["include_bounce"] == "true"
        return json_response([SAMPLE_INBOUND_SUMMARY])

    client = make_client("https://test.com", "key", handler)
    emails = client.inbound_emails.list(include_bounce=True)
    assert len(emails) == 1
    client.close()
